...
export default function App() {
  ...
  const handleSelectMunicipality = (muniName: string, clickedLat?: number, clickedLon?: number) => {
    ...
    if (system) {
      const lat = clickedLat !== undefined ? clickedLat : m.lat;
      const lon = clickedLon !== undefined ? clickedLon : m.lon;

      // Calculate coordinates of the target position in its recommended local system
      const gridCoords = geodeticToGrid(lat, lon, system);
      ...
      // Also set comparative map analysis coordinates so they match the clicked spot!
